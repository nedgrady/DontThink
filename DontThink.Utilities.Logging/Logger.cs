...
namespace DontThink.Utilities.Logging
{
            ...
            IDisposable
    {
        ...
        /// <param name="minLogLevel">The minimum log level required for a logged item to be actually saved. (i.e. logger will drop any items lower then the min log level.)</param>
        public Logger(
            ...
            LogLevel minLogLevel = LogLevel.Error)
        {
            ...
            Thread workerThread = new Thread(async () => { await LoggerWorker().ConfigureAwait(true); });
            workerThread.Start();
        }
